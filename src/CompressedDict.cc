/*
	A compressed Dictionary stores words to be used in a document or documents.
	For a universal dictionary, compression isn't as important because it is
	amortized across many documents, but all the words not found in this 
	dictionary must be defined in each document.

	For a document composed solely of words in a dictionary, compression can be
	very high. The larger the word set, the more bits in each word, but given
	that there are far fewer legal words than sequences of letters, the 
	compression is high. Also, parametric compression lets us compress common
	sequences of common words, even when containing uncommon words.

	However, most documents contain words that are not stored in any dictionary.
	These may be neologisms, typos, names that are uncommon and hence not deemed
	worthy. For efficient representation of documents, it is necessary to encode
	these words as well.
	...
	Instead, a compression mechanism for English to be discussed in the future
	requires a dictionary to be defined at the top of the document containing all
	words in the document that are not in the main dictionary. Because
	the dictionary can be stored in sorted order, it can be more
	efficiently encoded. All letters starting with a for example, can be
	stored without the a.

	Compressed format:
	startchar endchar bitvector of which characters are used in the dictionary
	then for each character in this top list, 
	bit for isWord (if true, then it is a word) a 16-bit offset for where the 
	children of this node start, and a bitvector of letters used.
...
split to 5 bits max = 32
	...
	example: aa, aal, aalii, aam, aani, aardvark, aardwolf, aaronic, aaronical, aaronite, aaronitic, aaru

	l END lii END m END ni END rdvark END rdwolf END ronic END ronical END ronite END ronitic END ru END
...
bdeh END bua END c END ca END cate END cay END cinate END cination END
	...
	0 00000000000000000100000000   (a is not a word, that's in the main dictionary, only next letter is r)
  0 00001000000000000000000000   (d is not a word, next letter is e)
  0 00000000100000000000100000   (f is not a word, next letters i and u)
  ...
  0 0 gentia                     (0=ar is not a word, 0=trie node
	...
	words.
	13 letters and tokens fit in each 64 bit word with just over 1 bit to spare
...
public:
	...
	bool comparePrefix(const char prefix[], const char* dict, uint32_t prefixLen, uint32_t dictIndex) {
  ...
  uint32_t countThisPrefix(char prefix[], uint32_t prefixLen) {
		for (uint32_t j = i; j <= len; j++) {
			if (comparePrefix(prefix, p+j, prefixLen)) {
				...
				if (countWordsThisPrefix > maxNodeSize)
				  return countWordsThisPrefix; // too big, so stop counting and recurse
	    else
				break;
			//skip to next word
	...
	void recursiveFindPrefix(char prefix[], uint32_t prefixLen, const char dict[], uint32_t dictLen, uint32_t dictIndex) {
		// find each letter after the current prefix, ie for a --> aa, ab, ac, ad ...
		if (countThisPrefix(prefix, prefixLen, dict, dictLen, dictIndex) > maxNodeSize) {
				for (uint32_t i = dictIndex; i < dictLen; ) {
					if (comparePrefix(prefix, dict, prefixLen, i)) {
						prefix[prefixLen] = dict[i+prefixLen];
				  	i = recursiveFindPrefix(prefix, prefixLen+1, dict, dictLen, dictIndex);
	...
	void loadASCII(const char filename[]) {
		...
		*/
		...
		// start with 2 letter prefixes because we know that every starting letter
		...
		skipspace:
			...
			for (int j = 0; j < prefixLen; j++) {
				if (p[i] <= ' ') { // this word isn't long enough for current prefix
					cout << "prefix error! " ;
					for (int k = i-j; p[k] >= ' '; k++)
						cout << p[k];
					cout << '\n';
			...
			//if > k, then create a trie node, split and repeat for each child
			uint32_t bitmask = 0; // track how many letters are used after prefix
			...
			// by now we have counted how many words start with this prefix, ie aba
		  if (countWordsThisPrefix > maximumWords) {
				...
				uint8_t nextLetter = p[j];

				???

				if (nextLetter >= 'a' && nextLetter <= 'z') {
					bitmask |= (1 << (nextLetter - 'a')); // set bit as having been seen
					while (j < len && p[j] >= ' ')
						j++; // skip to end of word
					while (j < len && p[j] <= ' ')
						j++; // skipspaces
		...
		endPrefix:
			if (countWordsThisPrefix < desiredCap) {
				...
				for (int j = i+prefixLen; j < endPrefix; j++) {
					writeOneWord(current, power, buffer, bufIndex, words[j] - 'a');
			}
	...
	void writeCompressed(ofstream& out) {
		...
		for (uint32_t i = 0; i < offsets.size()-1; i++) {
			for (uint32_t j = offsets[i]; j < offsets[i+1]; j++) {
				writeOneWord(out, current, power, buffer, bufIndex, words[j] - 'a');
			}
			// now write the code indicating termination of word
			writeOneWord(out, current, power, buffer, bufIndex, 26);
		}
